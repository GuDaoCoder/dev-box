...
说明：M4 实现已于 2026-09-19 验收。三平台编译与安装包生成已进入 CI；本机已生成 macOS `.app` 与 DMG。安装、升级和卸载作为首次发布操作项，仍需在对应平台执行后勾选。

## 独立扩展：Java Snippet Runner

状态：**J-M3 已完成，待验收**

- [x] 独立 `devbox-java-runner` Git 项目、Java 语法高亮和中英文界面
- [x] `java:execute` Plugin API、安装安全警告和一次性用户手势校验
- [x] 系统 JDK 17+ / JShell 发现、固定启动参数和结构化结果
- [x] 64 KiB 输入、5 秒超时、256 KiB 输出和单插件单并发限制
- [x] 正常输出、语法错误、运行异常、死循环和身份伪造回归
- [x] J-M3 可重复构建的 ZIP、安装授权、启停、执行、语言切换和卸载集成验收
