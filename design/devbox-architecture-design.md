...
11. 下一阶段规划平台内置轻量文本编辑器：用户主动选择目录后，主程序仅在该目录范围内展示树形文件列表并读写文本；不把文件系统能力开放给插件。界面设计详见 [统一界面设计规范](./devbox-ui-design-spec.md#75-轻量文本编辑器规划尚未实现)。
...
- 不向插件开放任意 Shell、任意文件系统或任意网络访问；规划中的内置文本编辑器只能访问用户主动选择的目录。
...
### 5.1 轻量文本编辑器（下一阶段，尚未实现）

文本编辑器属于主程序内置功能，不是 ZIP 插件，也不是 Java Runner。它复用 Shell 的二级功能树和外层 Tab，在功能页内部增加目录树与文件标签。目录选择由系统对话框发起，Rust Host 对用户所选根目录执行范围校验、按需列目录、读取文本和安全保存；前端只接收必要的文件元数据与内容。不得通过该能力为插件提供任意路径读写接口，不跟随越界符号链接。文件标签、未保存缓冲区和查找状态仅存在当前会话，不在重启后恢复。

编辑器首版面向常见文本格式的高亮与查找/替换，不包括工程编译、代码执行、语言服务器、Markdown 实时预览或项目范围替换。扩展名映射、布局、快捷键、文件状态和异常处理以 [统一界面设计规范第 7.5 节](./devbox-ui-design-spec.md#75-轻量文本编辑器规划尚未实现) 为准；性能和权限边界应在实现前落实为独立开发任务与测试。
