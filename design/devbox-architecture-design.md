...
DevBox 是本地优先、可扩展的桌面开发工具箱。JSON、XML、SQL 格式化、Timestamp、Encoding、UUID/Hash 是平台基础功能，随应用发布；第三方能力通过用户主动添加本地 ZIP 插件扩展。

当前基线：

1. 桌面容器采用 Tauri 2，界面采用 React + TypeScript，特权能力由 Rust Host 提供。
2. JSON、XML、SQL 格式化、Timestamp、Encoding、UUID/Hash 直接维护在 `dev-box`，不作为插件安装或更新。SQL 仅本地排版，不连接或执行数据库。
3. 平台不内置 Java 编辑器/Runner；Java 代码片段能力由 `devbox-plugins/plugins/java-runner` 官方插件提供，平台只提供受控 JShell Host API。JWT、正则测试和 HTTP Client 仍不实现。
...
11. 平台内置轻量文本编辑器：用户主动选择目录后，主程序仅在该目录范围内展示树形文件列表并读写文本；不把文件系统能力开放给插件。界面设计详见 [统一界面设计规范](./devbox-ui-design-spec.md#77-轻量文本编辑器)。
...
- 内置 JSON、XML、SQL 格式化、时间戳、编码转换、UUID 与 SHA-2 摘要功能。
...
- 不向插件开放任意 Shell、任意文件系统或任意网络访问；内置文本编辑器只能访问用户主动选择的目录。
...
flowchart TB
    subgraph Desktop["dev-box · Tauri 桌面应用"]
        Shell["App Shell\n二级功能树 + Tab 工作区"]
        Builtins["内置工具\nJSON / XML / SQL / Timestamp / Encoding / UUID-Hash"]
        Center["插件管理\n本地 ZIP"]
...
内置工具不经过 Plugin Installer，不显示插件签名、版本和权限：

| 功能 | 分类 | 核心能力 |
|---|---|---|
| JSON | 数据 | 格式化、压缩、校验、递归键排序、2 MiB 上限 |
| XML | 数据 | 格式化、保守压缩、格式校验；拒绝 DTD 与实体声明 |
| SQL | 数据 | 多方言本地格式化、关键字大小写和缩进；不连接或执行数据库 |
...
文本编辑器属于主程序内置功能，不是 ZIP 插件，也不是 Java Runner。它复用 Shell 的二级功能树和外层 Tab，在功能页内部增加目录树与文件标签。目录选择由系统对话框发起，Rust Host 对用户所选根目录执行范围校验、按需列目录、读取/保存文本和受控重命名；前端只接收必要的文件元数据与内容。读写支持 UTF-8、UTF-8 BOM、GBK 和带 BOM 的 UTF-16 LE/BE；切换保存编码必须等用户显式保存，无法无损转换时阻止写入。重命名只针对根目录内的单个文件，拒绝非法名称和同名覆盖。不得通过该能力为插件提供任意路径读写接口，不跟随越界符号链接。仅在本地保存上次目录路径、内部文件标签顺序和活动文件路径；再次打开编辑器时由 Host 重新读取磁盘内容。未保存缓冲区和查找状态不持久化，外层功能 Tab 仍不在重启后恢复。

编辑器首版面向常见文本格式的高亮与查找/替换，不包括工程编译、代码执行、语言服务器、Markdown 实时预览或项目范围替换。扩展名映射、布局、快捷键、文件状态和异常处理以 [统一界面设计规范第 7.7 节](./devbox-ui-design-spec.md#77-轻量文本编辑器) 为准。主程序通过独立的目录范围校验与文件读写命令实现权限边界；编辑器和各语言模块按需加载。
